...
<%if(request.getSession().getAttribute("order").equals("View all order")){%>
	<sql:query dataSource="${ds}" var="res">
			select Food_name, Food_price from resnew.food where Food_type='Appetizer';
	</sql:query>
<%}else if(request.getSession().getAttribute("order").equals("Remove order")){%>

<%}else if(request.getSession().getAttribute("order").equals("Checkout order")){%>
